package com.github.maeda6uiui.bd1toobj;

import org.apache.commons.cli.*;

public class BD1ToOBJMain {
    private static String VERSION_STR = "BD1ToOBJ v2.0.0";

    public static void main(String[] args) {
        // コマンドライン引数を解析するための設定
        var options = new Options();

        var optionI = new Option("i", "input-filepath", true, "Input filepath");
        var optionO = new Option("o", "output-filepath", true, "Output filepath");
        var optionZ = new Option("z", "invert-z", false,
                "Inverts the model with respect to the Z-axis");
        var optionF = new Option("f", "flip-v", false, "Flips texture V-coordinate");
        ...
        if (inputFilepath == null) {
            System.err.println("Input filepath must be specified.");
            return;
        }
        if (outputFilepath == null) {
            System.err.println("Output filepath must be specified.");
            return;
        }
